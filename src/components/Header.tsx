  ...
  return (
    <div className="relative bg-light-primary dark:bg-dark-primary">
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        <div className="flex items-center justify-between border-b-2 border-light-secondary dark:border-dark-secondary py-6 md:justify-start md:space-x-10">
          <div className="flex justify-start lg:w-0 lg:flex-1">
            <a href="https://medusanet.xyz">
              <span className="sr-only">Medusa</span>
              <img className="h-12 w-auto sm:h-24 rounded-full" src="/logo.png" alt="" />
            </a>
          </div>
          <h1 className="text-5xl font-light dark:text-white hidden sm:flex">{APP_NAME}</h1>

          <div className="items-center justify-end flex flex-1 lg:w-0 space-x-3">
            <button
              disabled={!address}
              onClick={handleFaucet}
              className="text-base font-medium text-black dark:text-white hover:text-light-secondary dark:hover:text-dark-secondary transition-colors disabled:cursor-not-allowed disabled:opacity-25"
            >
              Faucet
            </button>
            <ThemeSwitcher />
